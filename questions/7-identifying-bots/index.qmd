...
databases:
  - name: bots_q7
    path: "https://raw.githubusercontent.com/AmitLevinson/sql-website/main/questions/7-identifying-bots/q7.sql"
...
התרגיל נלקח מתוך הרצאה שהעברתי על מה זה להיות דאטה אנליסט בתחום ההונאות. מוזמנים להתנסות בעצמכם בקצת ממה שאני עושה ביום-יום :)

## שאלה

קיבלתם מייל מהמנהל שלכם עם תהייה לגבי קפיצה בהרשמות אצל לקוח מסוים שאתם עובדים איתו. הוא צירף למייל את הגרף הבא:

![](reg_count_plot.png)


אתם והמנהל חושדים שיש קבוצת בוטים שהצליחה לעקוף את המנגנונים שלכם ונרשמה יחד עם יוזרים רגילים. נסו למצוא את הקבוצה מתוך סך הנרשמים לחודש ינואר.
את הפלט הציגו באמצעות אגרגציה יומית של עמודה הסוכמת את מספר הבוטים שזיהיתם ועמודה שמראה את היוזרים 'שאינם בוטים'.


<details>
<summary>רמז</summary>
נסו לחפש דפוס בפרטים של היוזרים שתוכלו ליישם כלוגקיה לזהות קבוצה של יוזרים.
</details>
...
התרגיל מורכב ממדגם הרשמות של יוזרים באתר לחודש ינואר:
...
{{< video https://youtu.be/5Z8SACOqJD4 >}}
...
SELECT registration_date,
  SUM(CASE WHEN email LIKE SUBSTR(first_name, 1 ,1 ) || last_name || '__@%' THEN 1 END) as fraud_group,
  COUNT(*) - SUM(CASE WHEN email LIKE SUBSTR(first_name, 1 ,1 ) || last_name || '__@%' THEN 1 END) as Not_Group
FROM registrations_7
...
SELECT registration_date,
  SUM(CASE WHEN email LIKE LEFT(first_name, 1) + last_name+'__@%' THEN 1 ELSE 0 END) as fraud_group,
  COUNT(*) - SUM(CASE WHEN email LIKE LEFT(first_name, 1) + last_name+'__@%' THEN 1 ELSE 0 END) as Not_Group
FROM registrations_7
GROUP BY registration_date
ORDER BY registration_date
```
