...
עבור כל יוזר יש לחלץ את התשלום הראשון שקיבל ושני ציוני סכנת נטישה: הציון המאוחר ביותר שמתועד *לפני* קבלת התשלום הראשון, והציון המוקדם ביותר שמתועד *לאחר* התשלום הראשון.
...
CROSS apply (
  SELECT TOP 1 
    max(CASE WHEN first_payment > score_date then score_date END) OVER(PARTITION BY user_id) as score_date_before_paymet,
    min(CASE WHEN first_payment < score_date then score_date END) OVER(PARTITION BY user_id) as score_date_after_paymet
...
נסו ליצור פלט נוסף שבו במקום תאריכים מופיעים ציוני סכנת הנטישה (הציון לפני התשלום הראשון והציון מיד לאחר התשלום הראשון)
...
OUTER APPLY (
  SELECT top 1
   FIRST_VALUE(churn_score) OVER(ORDER BY CASE WHEN DATEDIFF(day, score_date, first_payment) > 0 THEN 1 ELSE 2 END, score_date desc) as score_before_payment,
   FIRST_VALUE(churn_score) OVER(ORDER BY CASE WHEN DATEDIFF(day, score_date, first_payment) < 0 THEN 1 ELSE 2 END, score_date asc)  as score_after_payment 
  from churn_scores_ag_4 scores
  where scores.user_id = user_payment.user_id
) AS churn_scores
```
