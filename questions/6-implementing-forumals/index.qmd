...
SELECT TOP 10 category,
	amount,
	(AMOUNT - MIN(AMOUNT) OVER (PARTITION BY category)) * 1.0 / 
						(MAX(AMOUNT) OVER (PARTITION BY category) - MIN(AMOUNT) OVER (PARTITION BY category)) * 100 AS amount_normalized
FROM items_q6
...
SELECT category,
	amount,
	amount_noramlized = (AMOUNT - MIN(AMOUNT) OVER (PARTITION BY category)) * 1.0 / 
						(MAX(AMOUNT) OVER (PARTITION BY category) - MIN(AMOUNT) OVER (PARTITION BY category)) * 100
FROM items_q6
order by category, amount_noramlized
```
